...
def init_agent(
    ...
    if not skip_memory:
        try:
            ...
            from agent.prefetch_knowledge import SilverbulletKnowledgeProvider as _KnowledgeProvider
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if _mp and _mp.is_available():
                    if not any(
                        getattr(p, "name", None) == getattr(_mp, "name", None)
                        for p in agent._memory_manager.providers
                    ):
                        agent._memory_manager.add_provider(_mp)

            # Always enable the knowledge provider so retrieval + post-save extraction
            # are part of the default runtime, regardless of the configured memory backend.
            _knowledge_provider = _KnowledgeProvider()
            if _knowledge_provider.is_available():
                if not any(
                    getattr(p, "name", None) == getattr(_knowledge_provider, "name", None)
                    for p in agent._memory_manager.providers
                ):
                    agent._memory_manager.add_provider(_knowledge_provider)
            ...
            _ra().logger.warning("Memory provider initialization failed: %s", _mpe)
